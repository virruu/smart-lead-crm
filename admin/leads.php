			...
			<div class="slcrm-lead-main">
				...
				<div class="slcrm-card">
					...
					<table class="slcrm-detail-table">
						...
						<?php if ( $lead->lead_action ) : ?>
						<tr><th><?php esc_html_e( 'Trigger Action', 'smart-lead-crm' ); ?></th><td><span class="slcrm-source-badge slcrm-source-<?php echo esc_attr( $lead->lead_source ); ?>"><?php echo esc_html( $helper->get_conversion_label( $lead->lead_action ) ); ?></span> <?php if ( $lead->form_name ) : ?><span style="color:var(--gray-400);font-size:12px;margin-left:6px;"><?php echo esc_html( $lead->form_name ); ?></span><?php endif; ?></td></tr>
						<?php endif; ?>
	...
	<table class="slcrm-table">
		...
		<tbody>
			...
			<tr>
				...
				<td style="color:var(--gray-400);font-size:12px;font-style:italic;"><?php if ( $lead->remarks ) : echo esc_html( wp_trim_words( $lead->remarks, 8, '…' ) ); elseif ( $lead->phone ) : ?><span class="dashicons dashicons-phone" style="font-size:11px;width:11px;height:11px;vertical-align:middle;font-style:normal;"></span> <?php echo esc_html( $lead->phone ); else : esc_html_e( 'Waiting for WhatsApp message…', 'smart-lead-crm' ); endif; ?></td>
